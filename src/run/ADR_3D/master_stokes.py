...
# Parameters for initializing the object (darcy = DarcyAdvection)
Da0  = 0.1 #0.1
Pe0  = 1.0e2 #1.0e2
alpha0= 1.0e-5 #0.1   # beta = alpha0/phi0
cfl0 = 0.1 #0.1
...
file_name      = "master_stokes"
#file_name      = "Da_%3.2f_Pe_%.1E"%(Da0,Pe0) #Automatically names the output files
...
# Output parameters
def output_write(Da,Pe,alpha,cfl,fname= output_dir + "/a_parameters.out"):
...
output_write(Da0,Pe0,cfl0,alpha0)
...
class BoundaryConcentration(Expression):
    ...
    def eval_cell(self, values, x, ufl_cell):
        #f = 0.05*(1.0-tanh(0.0/0.2))*(1.0+sin(2.0*x[1]*3.14))/2.0
...
darcy = DarcyAdvection(Da=Da0,Pe=Pe0,cfl=cfl0,alpha=alpha0)
...
# Define the source term from the function for the concentration elements, Qc only
...
while t - T < DOLFIN_EPS:
    ...
    a,L=darcy.stokes(W,mesh,sol_prev=sol_initial,dt=dt0,f1=S,K=0.1,zh=Constant((1.0,0.0,0.0)))
